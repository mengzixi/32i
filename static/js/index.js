...
//card样式
/*$(function () {
	$(document).on('click','.ceo-cat-switcher-z',function(){
        var ojb=$(this).parent().next().find('.ceo-active').find(".card-title-desc .title")
        for (var i = 0; i < ojb.length; i++){
            width_p=$(ojb).parent().parent().width();
            if(width_p>0){
                $(ojb).parent().css("width",$(ojb[i]).parent().parent().width())
            }else{
    ...
    if(window.screen.width>900){
        ...
        for (var i = 0; i < $(".card-title-desc .title").length; i++){
            width_p=$($(".card-title-desc .title")[i]).parent().parent().width();
            if(width_p>0){
                $($(".card-title-desc .title")[i]).parent().css("width",$($(".card-title-desc .title")[i]).parent().parent().width())
            }else{
        ...
        $(".card-item").hover(function () {
            $(this).find('.title').css({"margin-top":"-60px","background":"linear-gradient(to bottom,rgba(255,255,255,0.78) 0%,#fff 30%)",'z-index':9999})
            $(this).find('.ceo_freepath_description').css({"height":"60px","background":"linear-gradient(to bottom,rgba(255,255,255,0.78) 0%,#fff 30%)",'z-index':9999,'opacity':1})
        },function () {
            $(this).find('.card-title-desc').css('width',$(this).width())
            $(this).find('.title').css({"margin-top":"0px"})
            $(this).find('.ceo_freepath_description').css({"height":"0",'opacity':0})
        })
    }
})*/
...
$(document).on('click touchstart', '.btn-bigger-cover', function (event) {
    ...
    if (bigger_cover.hasClass('load-poster-img')) {
        $.ajax({
            ...
            if (data.s == 200) {
                bigger_cover.attr('src', data.src+'?'+Math.floor((Math.random() * 10000) + 1));
...
//商城下载
function ceoshop_download() {
    if ($(".head-login").length) {
        $(".head-login").trigger("click")
        UIkit.modal('#modal-login').show();
    }
    if ($(".erphpdown-down").length) {
        if($(".erphpdown-down-layui").length == 0 && $(".erphpdown-down").attr('href')){
            window.open($(".erphpdown-down").attr('href'))
        }
        $(".erphpdown-down").trigger("click");
    } else if ($(".erphpdown-buy:first").length) {
        $(".erphpdown-buy:first").trigger('click')
    } else if ($(".erphp-login-must").length) {

    } else {
    	UIkit.modal('#modal-buy-vip').show();
//         alert("下载出错,请联系客服反馈")
//         debugger
    }
}
